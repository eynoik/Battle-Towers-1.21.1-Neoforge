...
public final class TowerDestructionManager {
    ...
    public static void start(ServerLevel level, BlockPos golemPosition) {
        PENDING.offer(new Task(level.dimension(), golemPosition.immutable()));
    ...
    private static final class Task {
        ...
        private final BlockPos topCenter;
        private int age;
        private int destroyedFloors;

        private Task(ResourceKey<Level> dimension, BlockPos topCenter) {
        ...
        private boolean tick(ServerLevel level) {
            ...
            int y = topCenter.getY() - destroyedFloors * FLOOR_DISTANCE;
            level.playSound(null, topCenter.getX(), y, topCenter.getZ(), SoundEvents.GENERIC_EXPLODE, SoundSource.HOSTILE, 4.0F, 0.75F);
            level.explode(null, topCenter.getX() + 0.5D, y, topCenter.getZ() + 0.5D,
        ...
        private void spawnWarningParticles(ServerLevel level) {
            ...
            int y = topCenter.getY() - destroyedFloors * FLOOR_DISTANCE;
            double x = topCenter.getX() - 7 + level.random.nextInt(15) + level.random.nextDouble();
            double z = topCenter.getZ() - 7 + level.random.nextInt(15) + level.random.nextDouble();
        ...
        private void cleanFloor(ServerLevel level, int explosionY) {
            for (int x = -8; x < 8; x++) {
                for (int z = -8; z < 8; z++) {
                    for (int y = 1; y < 9; y++) {
                        BlockPos pos = new BlockPos(topCenter.getX() + x, explosionY + y, topCenter.getZ() + z);
