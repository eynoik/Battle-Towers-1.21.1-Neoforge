...
public final class TowerInteractionHandler {
    ...
    private static void awakenGuardianForChest(Player player, net.minecraft.core.BlockPos chestPos) {
        if (!(player.level() instanceof ServerLevel level)
                || !(level.getBlockState(chestPos).getBlock() instanceof ChestBlock)) {
            return;
        }

        TowerRegistrySavedData.get(level).containing(chestPos).ifPresent(record -> {
